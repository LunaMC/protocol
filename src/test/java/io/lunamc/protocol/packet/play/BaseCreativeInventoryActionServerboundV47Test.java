...
public class BaseCreativeInventoryActionServerboundV47Test extends AbstractPacketTest {

    private static final SlotData SLOT_DATA = Mockito.mock(SlotData.class);

    @Override
    protected Packet createPacket(boolean content) {
        BaseCreativeInventoryActionServerboundV47 packet = new BaseCreativeInventoryActionServerboundV47(mockDataAllocator());
        if (content) {
            packet.setSlot((short) 14);
